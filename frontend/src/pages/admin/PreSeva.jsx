...
import { MdShield, MdWarning, MdCheckCircle, MdSend, MdArrowUpward, MdArrowDownward, MdClose, MdPeople, MdMoneyOff, MdFlightTakeoff, MdRadar, MdBolt, MdTimeline } from 'react-icons/md';
        ...
        <div className="page-wrapper" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            {activeTab === 'missionControl' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    ...
                    {/* Live command banner */}
                    <div style={{ background: 'linear-gradient(135deg, rgba(239,68,68,0.08), rgba(139,92,246,0.06))', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 12, padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#EF4444', boxShadow: '0 0 10px #EF4444', animation: 'pulse 1.2s ease-in-out infinite', flexShrink: 0 }} />
                        <span style={{ fontFamily: 'monospace', fontSize: '0.82rem', fontWeight: 700, color: '#EF4444' }}>MISSION CONTROL LIVE</span>
                        <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', flex: 1 }}>Monitoring {missions.length} active predictions across {new Set(missions.map(m => m.state)).size} states — {missions.filter(m => m.status === 'pending').length} awaiting dispatch</span>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontFamily: 'monospace' }}>{new Date().toLocaleTimeString('en-IN')}</div>
                    </div>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(5,11,24,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: 20 }}>
                    <div style={{ background: '#0D1B2E', border: '1px solid rgba(239,68,68,0.4)', borderRadius: 16, padding: 28, maxWidth: 520, width: '100%', display: 'flex', flexDirection: 'column', gap: 16, boxShadow: '0 24px 64px rgba(0,0,0,0.7)' }}>
                        ...
                        <div className="form-group">
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.72rem', color: 'var(--text-muted)' }}><span>1 officer</span><span>20 officers</span></div>
                        </div>
                        <div className="responsive-grid-2" style={{ gap: 12 }}>
                            <div className="form-group">
                                <label className="form-label">Budget (₹)</label>
                                <input className="form-input" type="number" value={dispatchForm.budget} onChange={e => setDispatchForm(f => ({ ...f, budget: +e.target.value }))} />
                            </div>
                            <div className="form-group">
                                <label className="form-label">ETA (hours)</label>
                                <select className="form-input" value={dispatchForm.eta} onChange={e => setDispatchForm(f => ({ ...f, eta: e.target.value }))}>
                        ...
                        <div className="form-group">
                            ...
                            <textarea className="form-input" rows={3} placeholder="Action plan, priorities, escalation notes..." value={dispatchForm.note} onChange={e => setDispatchForm(f => ({ ...f, note: e.target.value }))} />
                        </div>
            ...
            {/* Only show below when on Predictions tab */}
            {activeTab !== 'missionControl' && <>
