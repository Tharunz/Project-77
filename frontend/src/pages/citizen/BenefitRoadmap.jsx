...
export default function BenefitRoadmap() {
    ...
    useEffect(() => {
        apiGetBenefitRoadmap(user?.id).then(res => { setRoadmap(res.data); setLoading(false); });
    }, []);
    ...
    const pct = Math.round((roadmap.completedSteps / roadmap.totalSteps) * 100);
        ...
        <div className="page-wrapper" style={{ display: 'flex', flexDirection: 'column', gap: 28, maxWidth: 760, margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', gap: 32, alignItems: 'center', flexWrap: 'wrap' }}>
                    <div>
                        ...
                        <div style={{ fontFamily: 'Space Grotesk', fontSize: '2.4rem', fontWeight: 900, color: 'var(--teal)' }}>
                            ₹{roadmap.totalPotentialBenefit.toLocaleString()}
                        </div>
                        <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: 4 }}>+₹5 lakh health cover</p>
                    </div>
                    <div style={{ flex: 1, minWidth: 200 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                            <span style={{ fontSize: '0.82rem', color: 'var(--text-secondary)' }}>Progress ({roadmap.completedSteps}/{roadmap.totalSteps} steps)</span>
                        ...
                        <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: 8 }}>
                            Complete {roadmap.totalSteps - roadmap.completedSteps} more steps to unlock full benefits
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {roadmap.steps.map((step, i) => {
                    ...
                    const isNext = !step.done && roadmap.steps.find((s, si) => !s.done && si === i);
                    return (
                        <div key={step.id} style={{ display: 'flex', gap: 0 }}>
                            ...
                            {/* Card */}
                            <div style={{
                                flex: 1, marginLeft: 16, marginBottom: i < roadmap.steps.length - 1 ? 16 : 0,
                                ...
                                <div style={{ padding: '14px 18px', cursor: !step.done ? 'pointer' : 'default', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                                            ...
                                            {!step.done && isNext && <span className="badge badge-pending">👉 Next Step</span>}
                                ...
                                {isActive && !step.done && (
                                    <div style={{ padding: '0 18px 16px', borderTop: '1px solid rgba(255, 255, 255, 0.12)', animation: 'fadeIn 0.3s ease' }}>
                                        ...
                                        <button className="btn-primary" style={{ fontSize: '0.82rem' }}>
                                            Apply for {step.scheme} <MdArrowForward />
                                        </button>
                                    </div>
                                )}
                            </div>
